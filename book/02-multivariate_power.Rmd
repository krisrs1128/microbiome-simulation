...
```{r save_multivariate, eval = FALSE, echo = FALSE}
ggsave("~/Downloads/multivariate_evaluation_qualitative.png", width = 10, height = 8, dpi = 500)
```
...
```{r power-analysis-loop-soln, eval = TRUE}
for (i in seq_len(nrow(config))) {
...
```{r visualize-power, eval = TRUE}
# config |>
#   group_by(sample_size, n_null) |>
#   summarise(min = min(metrics), max = max(metrics), median = median(metrics)) |>
#   ggplot(aes(sample_size, fill = factor(n_null))) +
#     geom_ribbon(aes(ymin = min, ymax = max)) +
#     geom_line(aes(y = median))

# ggplot(config, aes(factor(sample_size), metrics, fill = factor(n_null))) +
#   geom_boxplot()

# ggplot(config, aes(interaction(n_null, factor(sample_size)), metrics, col = factor(n_null))) +
#   stat_boxplot()
