...
for( i in 3:dim(atlas)[1]){
 ...
 ~ZINBF())
   ...
   error_warning<-estimate(sim, control =gamlss.control(n.cyc = 20,
                                   mu.step = 0.1))|>
...
sim<-estimate(sim, control =gamlss.control(n.cyc = 20,
                                  mu.step = 0.1))
```
## Critique
...
ggplot(top_combined) +
  ...
  ylab ("")+  
  labs(fill='BMI group') + theme(legend.position = 'right')+
...
prop.accepted.H0<- sapply(c("lean", "overweight","obese"), 
                          function(group){
                            group_filter<- map(unique(combined$feature), 
                                               ~combined |> 
                                                 filter( feature==. 
                                                         & bmi_group==group))
                            adjpvalue <- sapply(1:length(select_i),
                                              function(i){tryCatch( 
                                                {kdetest.group(
                                                  group_filter[[i]])},
                                                error=function(e){NA})})|>
...
ggplot(power_fdr)+
  ...
  facet_wrap(~metric)+
  xlab("Sample size")+
  ylab("")+ scale_color_manual(values = wes_palette("Moonrise2", n = 3))+
  labs(color='Method') + theme(legend.position = 'right')
```
